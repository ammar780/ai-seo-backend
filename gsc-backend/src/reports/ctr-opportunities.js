...
export async function ctrOpportunities(gsc, {
  ...
  const opportunities = rows
    // Include position 0 (featured snippets) — they often have HIGH CTR potential
    ...
    .map((r) => {
      ...
      // Conservative upside: assume rewrite captures the realism factor of the gap.
  ...
  return {
    ...
    sections: [
      { type: 'note',
        text: `Benchmarks reflect 2026 post-AI-Overview CTR floors. Upside is conservative: ${THRESHOLDS.CTR_LIFT_REALISM * 100}% of the CTR gap × impressions, matching real A/B test outcomes. Queries already within ${THRESHOLDS.CTR_UNDERPERFORMANCE * 100}% of benchmark are excluded. If an AI Overview consistently appears for these queries, realistic upside is lower still — verify SERP appearance before rewriting. Click "AI: Rewrite Titles" below for optimized variants.` },
