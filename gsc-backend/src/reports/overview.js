...
import { updatesInRange } from '../seo-constants.js';
...
export async function overview(gsc, { days = 28 } = {}) {
  ...
  // Known Google algorithm updates inside this date range — chart can overlay them
  const algorithmUpdates = updatesInRange(range.current.startDate, range.current.endDate);
  ...
  return {
    ...
    scorecards: [
      ...
      { label: 'Avg Position', value: cur.position ? cur.position.toFixed(1) : '—', delta: prev.position ? -pctDelta(cur.position, prev.position) : null, deltaLabel: '↑ = improved rank' },
    ],
    sections: [
      ...(algorithmUpdates.length > 0 ? [{
        type: 'note',
        text: `⚠ ${algorithmUpdates.length} Google algorithm update${algorithmUpdates.length > 1 ? 's' : ''} during this period: ${algorithmUpdates.map((u) => `${u.name} (${u.date})`).join(' · ')}. These may explain ranking or traffic shifts.`,
      }] : []),
      { type: 'chart', chartType: 'line', title: 'Daily clicks & impressions',
        data: { labels: dailyRows.map((r) => r.keys[0]),
          ...
          annotations: algorithmUpdates.map((u) => ({ date: u.date, label: u.name, impact: u.impact })) } },
    ...
    rawData: { algorithmUpdates, totalClicks: cur.clicks, totalImpressions: cur.impressions },
  };
}
