...
export class GSCClient {
  ...
  async query(params) {
    ...
    const { startDate, endDate, dimensions = [], filters = [],
      rowLimit = 1000, maxRows = 25000, searchType = 'web', dataState = 'final' } = params;
    const allRows = [];
    let startRow = 0;
    while (allRows.length < maxRows) {
      ...
      const res = await this.api.searchanalytics.query({ siteUrl: this.siteUrl, requestBody: body });
...
export function comparisonRange(days = 28, lagDays = 3) {
  return {
...
export function yearOverYearRange(days = 28, lagDays = 3) {
  return {
    current: { startDate: daysAgo(lagDays + days - 1), endDate: daysAgo(lagDays) },
    previous: { startDate: daysAgo(lagDays + 365 + days - 1), endDate: daysAgo(lagDays + 365) },
    days,
  };
}
